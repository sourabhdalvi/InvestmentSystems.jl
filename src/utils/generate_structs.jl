...
=#
\"\"\"
    mutable struct {{struct_name}}{{{closing_constructor_text}}} <: {{supertype}}
        {{#parameters}}
        {{name}}::{{{data_type}}}
        {{/parameters}}
    end
...
function {{constructor_func}}({{#parameters}}{{^internal_default}}{{name}}{{#default}}={{default}}{{/default}}, {{/internal_default}}{{/parameters}}) {{#has_parametric}}where {{{closing_constructor_text}}}{{/has_parametric}}
...
function {{struct_name}}({{#parameters}}{{^internal_default}}{{name}}{{#default}}={{default}}{{/default}}, {{/internal_default}}{{/parameters}})
    {{struct_name}}{{parameteric_constructor_text}}({{#parameters}}{{^internal_default}}{{name}}, {{/internal_default}}{{/parameters}})
end
{{/has_parametric}}

{{#has_null_values}}
# Constructor for demo purposes; non-functional.
function {{constructor_func}}(::Nothing) {{#has_parametric}}where {{{closing_constructor_text}}}{{/has_parametric}}
    {{constructor_func}}(;
        {{#parameters}}
        {{^internal_default}}
...
{{/accessors}}
{{#parameters}}{{#forwarding}}# Forward methods of type {{data_type}} for field {{name}}.
forward(({{struct_name}}, :{{name}}), {{{data_type}}}){{/forwarding}}{{/parameters}}
"""
...
function generate_structs(directory, data::Vector; print_results = true)
    ...
    for item in data
        ...
        item["parameteric_constructor_text"] = ""
        if haskey(item, "parametric")
            has_parametric = true
            item["constructor_func"] *= "{"
            item["closing_constructor_text"] *= "{"
            item["parameteric_constructor_text"] *= "{"
            for p in item["parametric"]
                item["constructor_func"] *= "$(p["symbol"])"
                item["closing_constructor_text"] *= "$(p["symbol"]) <: $(p["type"])"
                item["parameteric_constructor_text"] *= "typeof($(p["field"]))"
                if !(last(item["parametric"]) == p)
                    item["constructor_func"] *= ", "
                    item["closing_constructor_text"] *= ", "
                    item["parameteric_constructor_text"] *= ", "
                end
        ...
        item["has_parametric"] = has_parametric
        item["has_forwarding"] = has_forwarding
